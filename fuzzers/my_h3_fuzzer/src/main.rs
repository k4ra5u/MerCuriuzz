...
use mylibafl::mutators::*;
use mylibafl::{
    executors::H3Executor, feedbacks::*, inputstruct::QuicStruct, mutators::quic_mutations,
    observers::*, schedulers::MCTSScheduler,
};
use nix::libc::srand;
...
struct Opt {
    ...
    #[arg(help = "weather to use check corpus module", long = "check-corpus")]
    check_corpus: bool,
...
pub fn main() {
    std::env::set_current_dir("/home/john/quic-fuzz/MerCuriuzz/fuzzers/my_h3_fuzzer");
    ...
    info!(
        ...
        first_name, first_port, first_cpu1, first_cpu2
    );
    info!(
        "Second project: {}, port: {}, cpus: {}, {}",
        second_name, second_port, second_cpu1, second_cpu2
    );
    ...
    let first_edge_shmem_id = unsafe { SHMEM_EDGE_MAP_FIRST.as_ref().unwrap().id().to_string() };
    let second_edge_shmem_id = unsafe { SHMEM_EDGE_MAP_SECOND.as_ref().unwrap().id().to_string() };
    ...
    let mut first_h3_semantic_observer = H3SemanticObserver::new("h3_semantic");
    ...
    let mut second_h3_semantic_observer = H3SemanticObserver::new("h3_semantic");
    ...
    let diff_h3_semantic_ob = DifferentialH3SemanticObserver::new(
        &mut first_h3_semantic_observer,
        &mut second_h3_semantic_observer,
    );
    ...
    let mut objective = feedback_or!(
        CrashFeedback::new(),
        NormalDifferFeedback::new(&diff_cpu_ob, &diff_mem_ob, &diff_pcap_ob, &diff_misc_ob),
        H3ResponseSemanticFeedback::new(&diff_h3_semantic_ob),
        H3CloseSemanticFeedback::new(&diff_h3_semantic_ob),
        H3QpackProgressFeedback::new(&diff_h3_semantic_ob),
    ...
    let mut first_executor = H3Executor::new(first_observers, shmem_provider.clone())
        ...
        .coverage_shmem_ids(first_edge_shmem_id.clone(), second_edge_shmem_id.clone())
        .envs(vec![
            ("__AFL_SHM_ID".to_string(), first_edge_shmem_id.clone()),
            ("__AFL_SHM_ID_SIZE".to_string(), MAP_SIZE.to_string()),
        ])
        .set_frame_seed(frame_rand_seed)
        .build_h3_conn_struct("k4ra5u.xx".to_owned(), first_port, "127.0.0.1".to_owned())
        .build();

    let mut second_executor = H3Executor::new(second_observers, shmem_provider.clone())
        ...
        .coverage_shmem_ids(first_edge_shmem_id.clone(), second_edge_shmem_id.clone())
        .envs(vec![
            ("__AFL_SHM_ID".to_string(), second_edge_shmem_id.clone()),
            ("__AFL_SHM_ID_SIZE".to_string(), MAP_SIZE.to_string()),
        ])
        .set_frame_seed(frame_rand_seed)
        .build_h3_conn_struct("k4ra5u.xx".to_owned(), second_port, "127.0.0.1".to_owned())
        .build();
    ...
    let mutator = StdScheduledMutator::new(h3_mutations());
