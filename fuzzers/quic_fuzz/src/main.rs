...
pub fn main() {
    ...
    let mut first_executor = QuicExecutor::new(
        first_observers,
    ...
    .envs(vec![
        ...
        ("OB_STRUCT".to_string(), unsafe {
            SHMEM_OB_RESPONSE_FIRST.as_ref().unwrap().id().to_string()
        }),
        ("QUIC_STRUCT".to_string(), unsafe {
            SHMEM_QUIC_STRUCT_FIRST.as_ref().unwrap().id().to_string()
        }),
    ])
    ...
    let mut second_executor = QuicExecutor::new(
        second_observers,
        shmem_provider.clone(),
    ...
    .envs(vec![
        ...
        ("__AFL_SHM_ID_SIZE".to_string(), MAP_SIZE.to_string()),
        ("OB_STRUCT".to_string(), unsafe {
            SHMEM_OB_RESPONSE_SECOND.as_ref().unwrap().id().to_string()
        }),
        ("QUIC_STRUCT".to_string(), unsafe {
            SHMEM_QUIC_STRUCT_SECOND.as_ref().unwrap().id().to_string()
        }),
    ])
    .set_frame_seed(frame_rand_seed)
