...
use mylibafl::{
    executors::NetworkQuicExecutor, feedbacks::*, inputstruct::QuicStruct, misc::*,
...
impl QuicObservers {
    pub fn new(
        ...
        QuicObservers {
            start_command: format!("/tmp/target/start.sh"),
            judge_command: format!("/tmp/target/judge.sh"),
            ip,
            port,
            pid,
            observers: RemoteObsData::new(
                normal_conn_ob,
                cc_time_ob,
                misc_ob,
                recv_pkt_num_ob,
    ...
    pub fn set_initial_mem_usage(&mut self) {
        ...
        let _ = mem_observer.capture_before_snapshot();
    }
    ...
    pub fn update_cpu_usage_obs(&mut self, cur_cpu_usages: Vec<f64>) -> bool {
        ...
        for cur_cpu_usage in cur_cpu_usages.iter() {
            ...
            let curr_process_time = get_process_cpu_time(cpu_usage_observer.pid)
                .expect("Failed to get process CPU time");
            let curr_cpu_times =
                get_cpu_time(&cpu_usage_observer.cpu_ids).expect("Failed to get CPU core times");
    ...
    pub fn process_quic_input(&mut self, rand_seed: u32, input_struct: InputStruct) {
        ...
        match &mut quic_st.conn {
            ...
            None => {
                for i in 0..5 {
                    quic_st =
                        QuicStruct::new("myserver.xx".to_owned(), self.port, self.ip.to_owned());
        ...
        self.update_recv_pkt_obs(
            total_recv_bytes as u64,
            total_recv_pkts,
            total_sent_bytes as u64,
            total_sent_pkts,
        );
