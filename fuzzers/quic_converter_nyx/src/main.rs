...
#[derive(Debug, Parser)]
#[command(name = "quic converter", about = "", author = "k4ra5u")]
struct Opt {
    #[arg(
        short = 'a',  // 显式设置为 'a',
        help = "host name",
    ...
    #[arg(short, help = "port", name = "p", default_value = "58440")]
    port: u16,

    // --cpuid 0,1,2,3
    #[arg(long, help = "CPU affinity", name = "cpuid", default_value = "0")]
    cpuid: String,
}
...
pub fn main() {
    ...
    let mut shm_data = Some(ShmData {
        mem: quic_shmem_provider
            .shmem_from_id_and_size(ShMemId::from_string(&input_shm_id), QUIC_SIZE)
            .unwrap(),
        id: input_shm_id,
    });
    let mut ob_shm_data = Some(ShmData {
        mem: quic_shmem_provider
            .shmem_from_id_and_size(ShMemId::from_string(&res_shm_id), OB_RESPONSE_SIZE)
            .unwrap(),
        id: res_shm_id,
    });
    ...
    let mut conn_observer = NormalConnObserver::new(
        "conn",
        "127.0.0.1".to_owned(),
        opt.port,
        "localhost".to_owned(),
    ...
    let mut quic_obs = QuicObservers::new(
        host,
        port,
        0,
        conn_observer,
