...
use quiche::{frame, packet, rand, range_buf::RangeBuf, Connection, ConnectionId, Error, Header};

fn gen_ping_frame() {
    let mut input_struct = InputStruct::new();
    input_struct = input_struct
        .set_pkt_type(packet::Type::Short)
        .set_recv_timeout(20)
        .set_send_timeout(1);
    ...
    let file_name = "corpus/pingframes";
...
fn gen_crypto_flood_frame() {
    ...
    input_struct = input_struct.calc_frames_cycle_len();
    let input_bytes = input_struct.serialize();
    let file_name = "corpus/crypto_flood";
    let mut file = File::create(file_name).unwrap();
    let des_input = quic_input::InputStruct_deserialize(&input_bytes);
    file.write_all(&input_bytes);
}
...
fn gen_cid_flood_frame() {
    ...
    frame_cycle1 = frame_cycle1.set_repeat_num(1);
    for i in 0..4096 {
        let mut conn_id = Vec::new();
        for j in 0..8 {
            let conn_id_byte = rand::rand_u8();
            conn_id.push(conn_id_byte);
        }
    ...
    input_struct = input_struct.calc_frames_cycle_len();
    let input_bytes = input_struct.serialize();
    let file_name = "corpus/cid_flood";
    let mut file = File::create(file_name).unwrap();
    let des_input = quic_input::InputStruct_deserialize(&input_bytes);
    file.write_all(&input_bytes);
}
...
fn gen_pc_flood_frame() {
    ...
    for i in 0..4096 {
        ...
        for j in 0..8 {
            rand_bytes[j] = rand::rand_u8();
        }
        let pc_frame = frame::Frame::PathChallenge { data: rand_bytes };
        frame_cycle1 = frame_cycle1.add_frame(pc_frame);
    }
...
fn gen_nci_huge_frame() {
    ...
    input_struct = input_struct.calc_frames_cycle_len();
    let input_bytes = input_struct.serialize();
    let file_name = "corpus/nci_huge_frames";
    let mut file = File::create(file_name).unwrap();
    let des_input = quic_input::InputStruct_deserialize(&input_bytes);
    file.write_all(&input_bytes);
}
...
fn gen_aioquic_rc_flood_frame() {
    ...
    let rc_frame = frame::Frame::RetireConnectionId { seq_num: 1 };
