...
struct WorkerState {
    prefix: String, // Txx
    msg: String,    // ip:port or ok/fail info
    len: u64,
    pos: u64,
    status: WStatus,
}
...
pub fn spawn_ui_thread(
    ...
    std::thread::spawn(move || {
        let _ = run_ui(threads, cols, log_lines, rx);
    })
}
    ...
    for vr in 0..visible_rows {
        ...
        for c in 0..cols {
            ...
            if idx < n {
                ...
                let line = format_worker_cell(ws, w as usize);
...
fn format_worker_cell(ws: &WorkerState, width: usize) -> String {
    // 目标格式（与 indicatif 类似）：
    ...
    if avail >= (min_msg + min_bar) {
        msg_w = (38usize).min(avail - min_bar);
        if msg_w < min_msg {
            msg_w = min_msg;
        }
        bar_w = avail - msg_w;
    } else if avail >= min_msg {
        msg_w = avail;
        bar_w = 0;
    } else {
        msg_w = avail;
