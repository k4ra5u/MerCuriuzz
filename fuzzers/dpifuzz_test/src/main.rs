...
struct Opt {
    ...
    #[arg(help = "first conn port", name = "first_port", default_value = "58443")]
    first_port: u16,
...
fn main() {
    ...
    let mut first_harness = start_harness(&opt.first_name, unsafe {
        SHMEM_EDGE_MAP_FIRST.as_ref().unwrap().id().to_string()
    });
    let mut second_harness = start_harness(&opt.second_name, unsafe {
    ...
    loop {
        ...
        info!(
            "fir:{:?}/{:?} sec:{:?}/{:?}",
            first_count, first_total, sec_count, sec_total
        );
