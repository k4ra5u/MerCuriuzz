...
impl InputStruct {
    ...
    pub fn calc_frames_cycle_len(mut self) -> Self {
        ...
        for frame_cycle in self.frames_cycle.iter() {
            ...
            self.cycles_len
                .push(frames_cycle_bytes.len() - current_framses_len as usize);
    ...
    pub fn deserialize(mut self, input: &[u8]) -> Self {
        ...
        for i in 0..number_of_cycles {
            let cycle_len =
                u64::from_le_bytes(input[25 + i * 8..33 + i * 8].try_into().unwrap()) as usize;
            ...
            for frame in &frames_cycle.basic_frames {
                ...
                match frame {
                    frame::Frame::Padding { len } => {
                        frames.push(frame::Frame::Padding { len: *len });
                    }
                    frame::Frame::Ping {
                        mtu_probe: mtu_probe,
                    } => {
                        match *mtu_probe {
                            ...
                            None => frames.push(frame::Frame::Ping { mtu_probe: None }),
                        }
                    }
                    frame::Frame::ACK {
                        ack_delay,
                    ...
                    frame::Frame::Crypto { data } => {
                        ...
                        frames.push(frame::Frame::Crypto { data: mutated_data });
                    }
                    ...
                    frame::Frame::MaxData { max } => {
                        ...
                        mutated_max = *max + 1;
                        frames.push(frame::Frame::MaxData { max: mutated_max });
                    }
                    ...
                    frame::Frame::MaxStreamsBidi { max } => {
                        ...
                        frames.push(frame::Frame::MaxStreamsBidi { max: mutated_max });
                    }
                    frame::Frame::MaxStreamsUni { max } => {
                        let mut mutated_max = *max;
                        mutated_max = *max + i as u64;
                        frames.push(frame::Frame::MaxStreamsUni { max: mutated_max });
                    }
                    ...
                    frame::Frame::PathChallenge { data } => {
                        ...
                        frames.push(frame::Frame::PathChallenge { data: mutated_data });
                    }
                    frame::Frame::PathResponse { data } => {
                        ...
                        // SystemRandom::new().fill( & mut mutated_data[..]).unwrap();
                        frames.push(frame::Frame::PathResponse { data: mutated_data });
                    }
                    ...
                    frame::Frame::Datagram { data } => {
                        let mut mutated_data = data.clone();
                        mutated_data = data.clone();
                        frames.push(frame::Frame::Datagram { data: mutated_data });
                    }
                    ...
                    frame::Frame::Others { data } => {
                        let mut mutated_data = data.clone();
                        mutated_data = data.clone();
                        frames.push(frame::Frame::Others { data: mutated_data });
                    }
                }
            }
        }
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        for frame_cycle in self.frames_cycle.iter() {
            ...
            res.extend_from_slice(
                &(frames_cycle_bytes.len() as u64 - current_framses_len).to_le_bytes(),
            );
            current_framses_len = frames_cycle_bytes.len() as u64;
        }
...
pub fn InputStruct_deserialize(input: &[u8]) -> InputStruct {
    ...
    for i in 0..number_of_cycles {
        let cycle_len =
            u64::from_le_bytes(input[26 + i * 8..34 + i * 8].try_into().unwrap()) as usize;
        cycles_len.push(cycle_len);
    }
