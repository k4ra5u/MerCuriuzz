    ...
    S: State + HasReward,
{
        ...
        OT: ObserversTuple<S>,
    {
        ...
        */
        ...
        // }
        recv_pkt_record =
            (recv_pkt_num_ob.recv_pkts as f64 / (recv_pkt_num_ob.send_pkts * 5) as f64).min(1.0);
        cpu_record = (cpu_ob.final_based_cpu_usage - cpu_ob.based_cpu_usage) as f64 / 100.0;
        mem_record =
            ((mem_ob.after_mem - mem_ob.before_mem) as f64 / (mem_ob.allowed_mem) as f64).min(1.0);
...
impl UCBFeedback {
    ...
    #[must_use]
    pub fn new(
        ucb_observer: &UCBObserver,
        cpu_observer: &CPUUsageObserver,
        mem_observer: &MemObserver,
        cc_time_observer: &CCTimesObserver,
        recv_pkt_num_observer: &RecvPktNumObserver,
    ) -> Self {
        Self {
