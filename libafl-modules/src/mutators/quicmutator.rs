    ...
    I: HasMutatorBytes,
{
    fn mutate(&mut self, state: &mut S, input: &mut I) -> Result<MutationResult, Error> {
        ...
        let mut changed_repeat_num = state
            .rand_mut()
            .between(base_repeat_num >> 1, base_repeat_num << 1);
    ...
    I: HasMutatorBytes,
{
    fn mutate(&mut self, state: &mut S, input: &mut I) -> Result<MutationResult, Error> {
        ...
        // warn!("frame:{:?}",frame);
        quic_corp.frames_cycle[selected_cycle]
            .basic_frames
            .push(frame);
    ...
    I: HasMutatorBytes,
{
    fn mutate(&mut self, state: &mut S, input: &mut I) -> Result<MutationResult, Error> {
        ...
        if frames_cycle_len == 0 || frames_len == 0 {
            error!(
                "frames_cycle_len:{:?},frames_len:{:?}",
                frames_cycle_len, frames_len
            );
            return Ok(MutationResult::Skipped);
        }
        let frame = match quic_corp.frames_cycle[selected_cycle].basic_frames[selected_frame]
            .clone()
        {
            ...
            frame::Frame::Ping { mtu_probe } => {
                if let Some(mtu_probe) = mtu_probe {
                    let changed_mtu_probe =
                        state.rand_mut().between(mtu_probe >> 1, mtu_probe << 1)
                            & 0x3FFFFFFFFFFFFFFF;
                    frame::Frame::Ping {
                        mtu_probe: Some(changed_mtu_probe),
                    }
                } else {
            ...
            frame::Frame::ACK {
                ...
                match change_type {
                    0 => {
                        let changed_ack_delay = state.rand_mut().between(
                            ack_delay as usize >> 1,
                            (ack_delay << 1).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::ACK {
                            ack_delay: changed_ack_delay as u64,
                            ranges: ranges.clone(),
                            ecn_counts: ecn_counts.clone(),
                        }
                    }
                    1 => {
            ...
            frame::Frame::ResetStream {
                ...
                match change_type {
                    0 => {
                        let mut changed_stream_id = state.rand_mut().between(
                            stream_id as usize >> 1,
                            (min(stream_id << 1, stream_id + 40)).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                        changed_stream_id = changed_stream_id - changed_stream_id % 4;
                        frame::Frame::ResetStream {
                            stream_id: changed_stream_id as u64,
                            error_code,
                            final_size,
                        }
                    }
                    1 => {
                        let changed_error_code = state.rand_mut().between(
                            error_code as usize >> 1,
                            (error_code << 1).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::ResetStream {
                            stream_id,
                            error_code: changed_error_code as u64,
                            final_size,
                        }
                    }
                    2 => {
                        let changed_final_size = state.rand_mut().between(
                            final_size as usize >> 1,
                            (min(final_size << 1, final_size + 40)).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::ResetStream {
                            stream_id,
                            error_code,
                            final_size: changed_final_size as u64,
                        }
                    }
                    _ => unreachable!(),
                }
            ...
            frame::Frame::Crypto { data } => {
                ...
                match change_type {
                    0 => {
                        let changed_start = state
                            .rand_mut()
                            .between(data.start >> 1, min(data.start << 1, data.start + 1000))
                            & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::Crypto {
                            data: RangeBuf {
                    ...
                    1 => {
                        let changed_pos = state
                            .rand_mut()
                            .between(data.pos >> 1, min(data.pos + 1000, data.pos << 1))
                            & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::Crypto {
                            data: RangeBuf {
                                data: data.data.clone(),
                    ...
                    2 => {
                        let changed_len = state
                            .rand_mut()
                            .between(data.len >> 1, min(data.len + 1000, data.len << 1))
                            & 0x3FFFFFFFFFFFFFFF;
                        // TODO
                        // frame::Frame::Crypto { data: RangeBuf { data: data.data.clone(), start: data.start, pos: data.pos, len: changed_len, off: data.off ,fin: data.fin } }
                        frame::Frame::Crypto {
                            data: RangeBuf {
                                data: data.data.clone(),
                                start: data.start,
                    ...
                    3 => {
                        let changed_off = state.rand_mut().between(
                            data.off as usize >> 1,
                            (min(data.off << 1, data.off + 1000)).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::Crypto {
                            data: RangeBuf {
                                data: data.data.clone(),
            ...
            frame::Frame::CryptoHeader { offset, length } => {
                ...
                match change_type {
                    0 => {
                        let changed_offset = state.rand_mut().between(
                            offset as usize >> 1,
                            (min(offset + 1000, offset << 1)).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                    ...
                    1 => {
                        let changed_length = state.rand_mut().between(
                            length as usize >> 1,
                            (min(length + 1000, length << 1)).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::CryptoHeader {
                            offset,
            ...
            frame::Frame::NewToken { token } => frame::Frame::NewToken { token },
            frame::Frame::Stream { stream_id, data } => {
                ...
                match change_type {
                    ...
                    1 => {
                        let changed_pos = state
                            .rand_mut()
                            .between(data.pos >> 1, min(data.pos << 1, data.pos + 1000))
                            & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::Stream {
                            stream_id,
                            data: RangeBuf {
                    ...
                    2 => {
                        let changed_len = state.rand_mut().between(data.len >> 1, data.len << 1)
                            & 0x3FFFFFFFFFFFFFFF;
                        // TODO
                        // frame::Frame::Stream { stream_id, data: RangeBuf { data: data.data.clone(), start: data.start, pos: data.pos, len: changed_len, off: data.off ,fin: data.fin } }
                        frame::Frame::Stream {
                            stream_id,
                            data: RangeBuf {
                                data: data.data.clone(),
            ...
            frame::Frame::StreamHeader {
                ...
                match change_type {
                    0 => {
                        let changed_offset = state.rand_mut().between(
                            offset as usize >> 1,
                            (min(offset << 1, offset + 1000)).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::StreamHeader {
                            stream_id,
                            offset: changed_offset as u64,
                    ...
                    1 => {
                        let changed_length = state.rand_mut().between(
                            length as usize >> 1,
                            (min(length << 1, length + 1000)).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::StreamHeader {
                            stream_id,
                            offset,
            ...
            frame::Frame::MaxData { max } => {
                let changed_max = state.rand_mut().between(
                    max as usize >> 1,
                    (min(max << 1, max + 10000)).try_into().unwrap(),
                ) & 0x3FFFFFFFFFFFFFFF;
                frame::Frame::MaxData {
                    max: changed_max as u64,
                }
            }
            frame::Frame::MaxStreamData { stream_id, max } => {
                let changed_max = state.rand_mut().between(
                    max as usize >> 1,
                    (min(max + 10000, max << 1)).try_into().unwrap(),
                ) & 0x3FFFFFFFFFFFFFFF;
            ...
            frame::Frame::DataBlocked { limit } => {
                let changed_limit = state.rand_mut().between(
                    limit as usize >> 1,
                    (min(limit + 10000, limit << 1)).try_into().unwrap(),
                ) & 0x3FFFFFFFFFFFFFFF;
                frame::Frame::DataBlocked {
                    limit: changed_limit as u64,
                }
            }
            ...
            frame::Frame::NewConnectionId {
                ...
                match changed_type {
                    0 => {
                        let changed_seq_num = state.rand_mut().between(
                            max(seq_num as usize, 2) - 2,
                            (seq_num + 2).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::NewConnectionId {
                            seq_num: changed_seq_num as u64,
                            retire_prior_to,
                            conn_id,
                            reset_token,
                        }
                    ...
                    1 => {
                        let changed_retire_prior_to = state.rand_mut().between(
                            max(retire_prior_to as usize, 2) - 2,
                            (retire_prior_to + 2).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::NewConnectionId {
                            seq_num,
                            retire_prior_to: changed_retire_prior_to as u64,
                            conn_id,
                            reset_token,
                        }
                    }
            ...
            frame::Frame::PathChallenge { data } => frame::Frame::PathChallenge { data },
            frame::Frame::PathResponse { data } => frame::Frame::PathResponse { data },
            frame::Frame::ConnectionClose {
                ...
                match change_type {
                    ...
                    1 => {
                        let changed_frame_type = state.rand_mut().between(
                            frame_type as usize >> 1,
                            (frame_type << 1).try_into().unwrap(),
                        ) & 0x3FFFFFFFFFFFFFFF;
                        frame::Frame::Padding { len: (100) }
            ...
            frame::Frame::HandshakeDone => frame::Frame::HandshakeDone,
            frame::Frame::Datagram { data } => frame::Frame::Datagram { data },
            frame::Frame::DatagramHeader { length } => {
                let changed_length = state
                    .rand_mut()
                    .between(length as usize >> 1, (length + 100).try_into().unwrap());
                frame::Frame::DatagramHeader {
                    length: changed_length,
                }
            }
            frame::Frame::Others { data } => frame::Frame::Others { data },
        };
    ...
    I: HasMutatorBytes,
{
    fn mutate(&mut self, state: &mut S, input: &mut I) -> Result<MutationResult, Error> {
        ...
        let frame = match quic_corp.frames_cycle[selected_cycle].basic_frames[selected_frame]
            .clone()
        {
            frame::Frame::Padding { len } => frame::Frame::Padding { len },
            frame::Frame::Ping { mtu_probe } => frame::Frame::Ping { mtu_probe },
            frame::Frame::ACK {
                ack_delay,
                ranges,
                ecn_counts,
            ...
            frame::Frame::Crypto { data } => {
                let mut data_data_len = data.data.as_ref().len();
                let changed_data_data_len = state
                    .rand_mut()
                    .between(min(data_data_len >> 1, 1199), min(data_data_len << 1, 1200));
                let mut data_data = vec![0u8; changed_data_data_len];
                for i in 0..min(data_data_len, changed_data_data_len) {
                    data_data[i] = data.data.as_ref()[i];
                }
                frame::Frame::Crypto {
                    data: RangeBuf {
                        data: DefaultBufFactory::buf_from_slice(&data_data),
                        start: data.start,
                        pos: data.pos,
                        len: changed_data_data_len,
                        off: data.off,
                        fin: data.fin,
                        _bf: Default::default(),
                    },
            ...
            frame::Frame::NewToken { token } => {
                ...
                let changed_token_len = state
                    .rand_mut()
                    .between(min(token_len >> 1, 1199), min(token_len << 1, 1200));
            ...
            frame::Frame::MaxData { max } => frame::Frame::MaxData { max },
            frame::Frame::MaxStreamData { stream_id, max } => {
                frame::Frame::MaxStreamData { stream_id, max }
            }
            frame::Frame::MaxStreamsBidi { max } => frame::Frame::MaxStreamsBidi { max },
            frame::Frame::MaxStreamsUni { max } => frame::Frame::MaxStreamsUni { max },
            frame::Frame::DataBlocked { limit } => frame::Frame::DataBlocked { limit },
            frame::Frame::StreamDataBlocked { stream_id, limit } => {
                frame::Frame::StreamDataBlocked { stream_id, limit }
            }
            frame::Frame::StreamsBlockedBidi { limit } => {
            ...
            frame::Frame::StreamsBlockedUni { limit } => frame::Frame::StreamsBlockedUni { limit },
            frame::Frame::NewConnectionId {
                seq_num,
                retire_prior_to,
                conn_id,
            ...
            frame::Frame::ConnectionClose {
                ...
                let changed_reason_len = state
                    .rand_mut()
                    .between(min(reason.len() >> 1, 999), min(reason.len() << 1, 1000));
            ...
            frame::Frame::Datagram { data } => {
                let changed_data_len = state
                    .rand_mut()
                    .between(min(data.len() >> 1, 1199), min(data.len() << 1, 1200));
            ...
            frame::Frame::DatagramHeader { length } => frame::Frame::DatagramHeader { length },
            frame::Frame::Others { data } => frame::Frame::Others { data },
        };
    ...
    I: HasMutatorBytes,
{
    fn mutate(&mut self, state: &mut S, input: &mut I) -> Result<MutationResult, Error> {
            ...
            .clone()
        {
            ...
            frame::Frame::Crypto { data } => {
                let data_data_len = data.data.as_ref().len();
                ...
                match change_type {
                    ...
                    1 => {
                        ...
                        frame::Frame::NewConnectionId {
                            ...
                            retire_prior_to,
                            conn_id,
                            reset_token: changed_reset_token,
                        }
                    }
                    _ => unreachable!(),
                }
    ...
    I: HasMutatorBytes,
{
    fn mutate(&mut self, state: &mut S, input: &mut I) -> Result<MutationResult, Error> {
        ...
        let stream_frame = frame::Frame::Stream {
            stream_id: 8,
            data: RangeBuf {
                data: DefaultBufFactory::buf_from_slice(&header_block),
                start: 0,
                pos: 0,
                len: header_len,
                off: 0,
